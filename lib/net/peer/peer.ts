...
export class Peer extends events.EventEmitter {
  ...
  public fourtwenty: BoundProtocol | undefined
   ...
   * @example
   *
   * await peer.bindProtocol(fourtwentyProtocol, sender)
   * // Example: Directly call message name as a method on the bound protocol
   * const headers1 = await peer.fourtwenty.getBlockHeaders(1, 100, 0, 0)
   * // Example: Call request() method with message name as first parameter
   * const headers2 = await peer.fourtwenty.request('getBlockHeaders', 1, 100, 0, 0)
   * // Example: Call send() method with message name as first parameter and
   * // wait for response message as an event
   * peer.fourtwenty.send('getBlockHeaders', 1, 100, 0, 0)
   * peer.fourtwenty.on('message', ({ data }) => console.log(`Received ${data.length} headers`))
   */
