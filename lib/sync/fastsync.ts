...
export class FastSynchronizer extends Synchronizer {
   ...
   * @return {boolean}
   */
  // TODO: Correct logic for return type (b/c peer.fourtwenty is not boolean)
  // "Type 'BoundProtocol | undefined' is not assignable to type 'boolean'"
  syncable(peer: Peer): boolean {
    return peer.fourtwenty !== undefined
  }
  ...
  best(): Peer | undefined {
    ...
    for (const peer of peers) {
      if (peer.fourtwenty && peer.fourtwenty.status) {
        const td = peer.fourtwenty.status.td
        if (
          (!best && td.gte((this.chain.blocks as any).td)) ||
          (best && best.fourtwenty && best.fourtwenty.status.td.lt(td))
        ) {
          best = peer
        }
      }
  ...
  async latest(peer: Peer) {
    // @ts-ignore
    const headers = await (peer.fourtwenty as BoundProtocol).getBlockHeaders({
      block: (peer.fourtwenty as BoundProtocol).status.bestHash,
      max: 1,
    })
    return headers[0]
  }
