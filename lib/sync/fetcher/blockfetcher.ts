...
import { Block, BlockBodyBuffer } from '@fourtwentyjs/block'
...
export class BlockFetcher extends Fetcher {
  ...
  async request(job: any): Promise<any> {
    ...
    const headers = await peer.fourtwenty.getBlockHeaders({ block: first, max: count })
    const bodies = await peer.fourtwenty.getBlockBodies(headers.map((h: any) => h.hash()))
  ...
  peer(_job: any): Peer {
    return this.pool.idle((p: any) => p.fourtwenty)
  }
}
