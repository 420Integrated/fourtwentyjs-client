...
import { toBuffer, stripHexPrefix, BN } from 'fourtwentyjs-util'
...
export class Fourtwenty {
  ...
  constructor(node: any) {
    const service = node.services.find((s: any) => s.name === 'fourtwenty')
    this._chain = service.chain
    const fourtwentyProtocol = service.protocols.find((p: any) => p.name === 'fourtwenty')
    this.fourtwentyVersion = Math.max.apply(Math, fourtwentyProtocol.versions)
  ...
  protocolVersion(_params = [], cb: (err: null, val: string) => void) {
    cb(null, `0x${this.fourtwentyVersion.toString(16)}`)
  }
}
