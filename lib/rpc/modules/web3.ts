...
import { addHexPrefix, keccak, toBuffer } from 'fourtwentyjs-util'
...
export class Web3 {
  ...
  clientVersion(_params = [], cb: (err: null, version: string) => void) {
    ...
    const fourtwozerJsVersion = `FourtwentyJS/${packageVersion}/${platform()}/node${version.substring(1)}`
    cb(null, fourtwozeroJsVersion)
  }
