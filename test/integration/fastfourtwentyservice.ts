...
tape('[Integration:FastFourtwentyService]', async (t) => {
  ...
  t.test('should handle FOURTWENTY requests', async (t) => {
    const [server, service] = await setup()
    const peer = await (server as MockServer).accept('peer0')
    const headers = await (peer.fourtwenty as any).getBlockHeaders({ block: 1, max: 2 })
    ...
    const bodies = await (peer.fourtwenty as any).getBlockBodies([hash])
    t.deepEquals(bodies, [[[], []]], 'handled GetBlockBodies')
    await (peer.fourtwenty as any).send('NewBlockHashes', [[hash, new BN(2)]])
