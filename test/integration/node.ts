...
tape('[Integration:Node]', (t) => {
  ...
  t.test('should start/stop', async (t) => {
    ...
    await node.open()
    node.service('fourtwenty').synchronizer.interval = 100
    node.service('fourtwenty').emit('error', 'err0')
    await node.start()
    t.ok(node.service('fourtwenty').synchronizer.running, 'sync running')
    await node.stop()
    t.pass('node stopped')
  })
})
