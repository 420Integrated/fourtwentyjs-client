...
import { FastFourtwentyService } from '../../lib/service'

defaultLogger.silent = true

// TESTS FAILING: replace testdouble w/ something TS friendly?
tape.skip('[FastFourtwentyService]', (t) => {
  ...
  const FourtwentyProtocol = td.constructor()
  const LesProtocol = td.constructor()
  td.replace('../../lib/net/protocol/fourtwentyprotocol', FourtwentyProtocol)
  ...
  t.test('should initialize correctly', async (t) => {
    const service = new FastFourtwentyService()
    t.ok(service.synchronizer instanceof FastSynchronizer, 'fast mode')
    t.equals(service.name, 'fourtwenty', 'got name')
    t.end()
  })

  t.test('should get protocols', async (t) => {
    let service = new FastFourtwentyService()
    t.ok(service.protocols[0] instanceof FourtwentyProtocol, 'fast protocols')
    t.notOk(service.protocols[1], 'no light protocol')
    service = new FastFourtwentyService({ lightserv: true })
    t.ok(service.protocols[0] instanceof FourtwentyProtocol, 'fast protocols')
  ...
  t.test('should open', async (t) => {
    ...
    const service = new FastFourtwentyService({ servers: [server] })
    await service.open()
