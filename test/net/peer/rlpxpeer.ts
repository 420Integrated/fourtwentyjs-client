...
tape.skip('[RlpxPeer]', (t) => {
  const { DPT, FOURTWENTY, LES } = require('fourtwentyjs-devp2p')
  class RLPx extends EventEmitter {}
  ((<unknown>RLPx.prototype) as any).connect = td.func()
  td.replace('fourtwentyjs-devp2p', { DPT, FOURTWENTY, LES, RLPx })
  ...
  /*t.test('should compute capabilities', t => {
    const protocols = [ { name: 'fourtwenty', versions: [62, 63] }, { name: 'les', versions: [2] } ]
    const caps = RlpxPeer.capabilities(protocols).map(
      ({ name, version, length } : any) => ({ name, version, length }))
    t.deepEquals(caps, [
      { name: 'fourtwenty', version: 62, length: 8 },
      { name: 'fourtwenty', version: 63, length: 17 },
