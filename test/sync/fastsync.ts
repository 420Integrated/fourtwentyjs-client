...
tape('[FastSynchronizer]', (t) => {
  ...
  t.test('should initialize correctly', async (t) => {
    ...
    pool.emit('added', { fourtwenty: true })
  ...
  t.test('should get height', async (t) => {
    const pool = new PeerPool()
    const sync = new FastSynchronizer({ pool })
    const peer = { fourtwenty: { getBlockHeaders: td.func(), status: { bestHash: 'hash' } } }
    const headers = [{ number: 5 }]
    td.when(peer.fourtwenty.getBlockHeaders({ block: 'hash', max: 1 })).thenResolve(headers)
  ...
  t.test('should find best', async (t) => {
    ...
    const peers = [
      { fourtwenty: { status: { td: new BN(1) } }, inbound: false },
      { fourtwenty: { status: { td: new BN(2) } }, inbound: false },
    ]
    sync.pool = { peers }
    sync.forceSync = true
    td.when(sync.height(peers[0])).thenDo((peer: any) => Promise.resolve(peer.fourtwenty.status.td))
    td.when(sync.height(peers[1])).thenDo((peer: any) => Promise.resolve(peer.fourtwenty.status.td))
